...
{
	vec3 I = lights[i].dir;                         // Light source direction to P (Assume Parallel light source)
    ...
    if(typeOfColor == 0)
    {
	    ...
	    {
		    return lights[i].La * current_Ka * lights[i].color;
	    }
    }
    else
    {
		...
		{
			I = normalize(lights[i].position - P);
        ...
        if(typeOfColor == 1) //diffuse
        {
		    ...
		    return ((lights[i].Ld * current_Kd * dotProduct_IN) * (lights[i].color * current_Color_diffuse));
        }
        else if(typeOfColor == 2) //specular
        {
        	/* Calcualte specular */
		    float dotProduct_RV = max(0, dot(R, V));
		    dotProduct_RV = pow(dotProduct_RV, COS_ALPHA);
		    return (lights[i].Ls * current_Ks * dotProduct_RV) * (lights[i].color * current_Color_specular);
        }
    }
}
    ...
    else // draw shading algos
    {
        ...
        else //  FLAT / GOUROUD / PHONG
        {
            ...
            if(algo_shading == 1) //flat shading
            {
        		...
        		colorOfVertex = clamp( (Ia_total + Id_total + Is_total), vec3(0.0), vec3(1.0));
                resultPosition = projection * resultPosition;
            }
